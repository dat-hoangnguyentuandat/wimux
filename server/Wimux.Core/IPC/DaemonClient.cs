...
namespace Wimux.Core.IPC;

public sealed class DaemonClient : IDisposable
{
    private const string PipeName = "wimux-daemon";
    ...
    private static string? FindDaemonExecutable()
    {
        ...
        var candidate = Path.Combine(appDir, "wimux-daemon.exe");
        ...
        try
        {
            ...
            if (publishDir != null)
            {
                candidate = Path.Combine(publishDir.FullName, "wimux-daemon-win-x64-single", "wimux-daemon.exe");
                if (File.Exists(candidate)) return candidate;
            }
        }
        catch
        ...
        //    appDir is e.g. .../src/Wimux/bin/Debug/net10.0-windows10.0.17763.0/
        //    daemon is at   .../src/Wimux.Daemon/bin/Debug/net10.0-windows/wimux-daemon.exe
        try
        {
            ...
            if (dir != null)
            {
                var daemonBin = Path.Combine(dir.FullName, "Wimux.Daemon", "bin");
                if (Directory.Exists(daemonBin))
                {
                    ...
                    if (Directory.Exists(configDir))
                    {
                        ...
                        {
                            candidate = Path.Combine(tfmDir, "wimux-daemon.exe");
                            if (File.Exists(candidate)) return candidate;
                        }
                    }

                    foreach (var exe in Directory.GetFiles(daemonBin, "wimux-daemon.exe", SearchOption.AllDirectories))
                        return exe;
                }
            }
        }
    ...
    public static void LogDaemon(string message)
    {
        ...
        try
        {
            var logDir = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "wimux");
