...
/// <summary>
/// Named pipe server for wimux CLI/API communication.
/// Windows equivalent of the Unix domain socket used by wimux on macOS.
/// Pipe name: \\.\pipe\wimux (or \\.\pipe\wimux-{tag} for tagged instances).
/// </summary>
public sealed class NamedPipeServer : IDisposable
{
    ...
    {
        _pipeName = string.IsNullOrEmpty(tag) ? "wimux" : $"wimux-{tag}";
    }
...
/// <summary>
/// Client for connecting to the wimux named pipe server (used by the CLI).
/// </summary>
public static class NamedPipeClient
{
    ...
    {
        var pipeName = string.IsNullOrEmpty(tag) ? "wimux" : $"wimux-{tag}";
