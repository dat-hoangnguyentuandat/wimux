...
/// <summary>
/// Opens a wimux web-terminal pane whose command is `codex` so the user can
...
/// forwards the committed rune into the wimux server which writes it
...
internal static class CodexLauncher
{
    ...
    internal static int Open(string[] codexArgs)
    {
        ...
        {
            Console.Error.WriteLine($"Failed to open a codex pane in wimux: {error}");
            return 1;
        }

        var target = $"{url}/#surface={surfaceId}&pane={paneId}";
        BrowserLauncher.Open(target);
        Console.WriteLine($"Opened codex in wimux ({paneId}).");
