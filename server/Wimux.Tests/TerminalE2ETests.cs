...
/// <summary>
/// Starts the real wimux-web Kestrel host as a child process and drives the
...
public sealed class RealServerFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var dll = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory,
            "..", "..", "..", "..", "Wimux.Web", "bin", "Debug", "net10.0-windows", "wimux-web.dll"));
        File.Exists(dll).Should().BeTrue($"web host must be built at {dll}");

        var stateDir = Path.Combine(Path.GetTempPath(), "wimux-realsrv-" + Guid.NewGuid().ToString("N"));
    ...
    private static async Task WaitForPortAsync(int port, TimeSpan timeout)
    {
        ...
        throw new TimeoutException($"wimux-web did not start on port {port}");
    }
}
...
public class TerminalE2ETests : IClassFixture<RealServerFixture>
{
    ...
    public async Task SpawnsShell_AndEchoesCommandOutput()
    {
        ...
        var output = await SendUntilAsync(ws, "Write-Output ('WIMUX'+'_RT_OK')\r", "WIMUX_RT_OK", TimeSpan.FromSeconds(20));
        output.Should().Contain("WIMUX_RT_OK");
    }
