...
public sealed class AgentRuntimeService : IDisposable
{
    ...
    /// <summary>
    /// Optional handler bridging agent "wimux" tool calls to the local command
...
""");
    ...
    private async Task<string> ExecutePipeCommandAsync(string command, Dictionary<string, string> args, CancellationToken ct)
    {
        ...
        if (handler == null)
            return JsonSerializer.Serialize(new { error = "wimux command handler unavailable" });
    ...
    private static string BuildSystemPrompt(AgentSettings settings, AgentPaneContext context, IReadOnlyList<AgentTool> tools)
    {
        var userSystemPrompt = string.IsNullOrWhiteSpace(settings.SystemPrompt)
            ? "You are a pragmatic engineering assistant running inside wimux. Keep responses concise and action-oriented."
...
Use wimux_pane_list before targeting non-focused panes.
