...
/// debugging enabled, in a dedicated profile and pushed off-screen so wimux can show it
...
public sealed class ChromeCdpProvider : IBrowserProvider
{
    ...
    // The browser process wimux launched (if any), so we can fully quit it later.
    ...
    /// Chromium also stores session-restore data there. If wimux kills the hidden
