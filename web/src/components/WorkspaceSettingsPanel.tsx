...
export function WorkspaceSettingsPanel({ workspace, onClose }: Props) {
  ...
  return (
    <div className="wimux-panel wimux-workspace-settings">
      <div className="wimux-panel-toolbar">
        <div className="wimux-panel-toolbar-row">
          <span className="wimux-panel-title">SSH / ENV</span>
          <span className="wimux-list-title">{workspace.name}</span>
      ...
      {tab === "env" && (
        <div className="wimux-panel-body">
          ...
          <div className="wimux-modal-actions"><button onClick={onClose}>Cancel</button><button className="primary" onClick={saveEnv}>Save</button></div>
        </div>
      )}
      {tab === "ssh" && (
        <div className="wimux-panel-body">
          <div className="wimux-ssh-list">
            {ssh.map((p) => (
              <div className="wimux-ssh-card" key={p.id}>
                <label className="wimux-field"><span>Name</span><input value={p.name} onChange={(e) => updateProfile(p.id, { name: e.target.value })} /></label>
                <label className="wimux-field"><span>User</span><input value={p.user} onChange={(e) => updateProfile(p.id, { user: e.target.value })} /></label>
                <label className="wimux-field"><span>Host</span><input value={p.host} onChange={(e) => updateProfile(p.id, { host: e.target.value })} /></label>
                <label className="wimux-field"><span>Port</span><input type="number" value={p.port} onChange={(e) => updateProfile(p.id, { port: Number(e.target.value) })} /></label>
                <label className="wimux-field wimux-ssh-identity"><span>Identity file</span><input value={p.identityFile ?? ""} onChange={(e) => updateProfile(p.id, { identityFile: e.target.value })} /></label>
                <div className="wimux-ssh-preview mono dim">{buildCommand(p)}</div>
                <button className="wimux-icon-btn wimux-ssh-remove" onClick={() => removeProfile(p.id)} title="Remove profile"><TrashIcon /></button>
              </div>
            ))}
          </div>
